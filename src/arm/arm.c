...
mraa_arm_platform()
{
    ...
    if (platform_type == MRAA_UNKNOWN_PLATFORM) {
        if (mraa_file_contains("/proc/device-tree/model", "s900"))
            ...
            platform_type = MRAA_96BOARDS;
        else if (mraa_file_contains("/proc/device-tree/model", PLATFORM_NAME_RADXA_ROCK_PI_S))
            platform_type = MRAA_RADXA_ROCK_PI_S;
    ...
    switch (platform_type) {
        ...
        case MRAA_96BOARDS:
            plat = mraa_96boards();
            break;
        case MRAA_RADXA_ROCK_PI_S:
            plat = mraa_radxa_rock_pi_s();
            break;
        case MRAA_RADXA_CM3:
            plat = mraa_radxa_cm3();
            break;
